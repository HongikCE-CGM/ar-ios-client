...
public class BallUIButtonCreator : MonoBehaviour
{
    ...
    void CreateModernButton()
    {
        ...
        // 하단 중앙, 오른쪽 버튼 위치 (더욱 더 위로)
        rt.anchoredPosition = new Vector2(260, 240);

        // 모던한 둥근 버튼 배경
        toggleButtonImage = btnGO.AddComponent<Image>();
        toggleButtonImage.color = new Color(0.3f, 0.8f, 0.3f, 0.8f); // 모던한 그린
        ...
        modernAnimator.SetColors(
            new Color(0.3f, 0.8f, 0.3f, 0.8f), // normal
            new Color(0.4f, 0.9f, 0.4f, 0.9f), // hover
            new Color(0.1f, 0.6f, 0.1f, 1.0f)  // press
        );
    }
    ...
    void OnToggleBall()
    {
        ...
        if (ballOn)
        {
            foreach (var f in FindObjectsByType<FeedUIButtonCreator>(FindObjectsSortMode.None))
                f.SetFeedOn(false);
            foreach (var s in FindObjectsByType<SleepUIButtonCreator>(FindObjectsSortMode.None))
                s.SetSleepOn(false);
        }
    }
    ...
    void UpdateButtonAppearance()
    {
        if (ballOn)
        {
            var activeColor = new Color(0.1f, 0.6f, 0.1f, 1f); // 더 진한 그린
            ...
            if (modernAnimator != null)
            {
                modernAnimator.SetColors(
                    activeColor,
                    new Color(0.2f, 0.7f, 0.2f, 1f), // hover
                    new Color(0.05f, 0.5f, 0.05f, 1f) // press
                );
            }
        }
        else
        {
            var normalColor = new Color(0.3f, 0.8f, 0.3f, 0.8f); // 원래 그린
